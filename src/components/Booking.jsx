...
const ResourceBooking = () => {
  ...
  const [activeTab, setActiveTab] = useState('new'); // 'new', 'your', 'all'
  ...
  const fetchAllBookings = async () => {
    ...
    try {
      ...
      setError('Failed to load community bookings. Please try again.');
    } finally {
      setBookingsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      <div className="max-w-6xl mx-auto p-4">
        <h2 className="text-3xl font-bold mb-6 text-gray-800">Resource Booking</h2>
            ...
            {activeTab === 'all' && (
              <div>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
                  <h3 className="text-xl font-semibold">Community Bookings</h3>
                ...
                {bookingsLoading ? (
                  <div className="flex justify-center items-center py-12">
                    <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-500"></div>
                    <span className="ml-2 text-gray-700">Loading community bookings...</span>
                  </div>
                ) : (
                  <>
                    {filterBookings(allBookings, activeBookingFilter, searchTerm).length === 0 ? (
                      <div className="bg-white text-center py-12 rounded-lg border border-gray-200">
                        <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        ...
                        {searchTerm ? (
                          ...
                          <p className="mt-1 text-sm text-gray-500">No upcoming community bookings</p>
                        ) : activeBookingFilter === 'past' ? (
                          <p className="mt-1 text-sm text-gray-500">No past community bookings</p>
                        ) : (
                      ...
                      <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-300">
                          ...
                          <tbody className="divide-y divide-gray-200 bg-white">
                            {filterBookings(allBookings, activeBookingFilter, searchTerm).map((booking) => {
                              const startTime = booking.startTime.toDate();
                              const endTime = booking.endTime.toDate();
                              const isPast = endTime < new Date();
                              const isToday = new Date().toDateString() === startTime.toDateString();
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    ...
                                    <div className="text-xs">{getDurationInHours(booking.startTime, booking.endTime)}</div>
                                  </td>
